...
class AiOpenOrCreateResult(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in prior_messages (list)
        _items = []
        if self.prior_messages:
            for _item_prior_messages in self.prior_messages:
                if _item_prior_messages:
                    _items.append(_item_prior_messages.to_dict())
            _dict['priorMessages'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "title": obj.get("title"),
            "priorMessages": [AiThreadMessageLike.from_dict(_item) for _item in obj["priorMessages"]] if obj.get("priorMessages") is not None else None
        })
        return _obj
