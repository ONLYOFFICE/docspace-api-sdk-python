...
class AiCreateProfileInput(BaseModel):
    """
    ...
    provider_type: AiProviderType = Field(description="Provider type for this profile. Use `external` to delegate all HTTP transport to  {@link  PlatformAdapter.externalFetch  }  while reusing an existing provider's response parser — see  {@link  Profile.basedOn }  for the format selector.", alias="providerType")
    ...
    headers: Optional[Dict[str, StrictStr]] = Field(default=None, description="Extra HTTP headers sent with every request to this provider. Merged into the SDK client's default headers; an explicit `Authorization` here wins over the one derived from  {@link  key  } . Honoured by the OpenAI-family providers.")
    ...
    use_proxy: Optional[StrictBool] = Field(default=None, description="Route every provider request through the host's `fetchProxy` instead of the global `fetch`. Useful when the host runs the widget in a sandbox without direct network access (CORS, custom auth, etc.). Has no effect when the  {@link  PlatformAdapter.fetchProxy  }  is not configured.", alias="useProxy")
    __properties: ClassVar[List[str]] = ["name", "providerType", "basedOn", "baseUrl", "key", "headers", "modelId", "reasoning", "capabilities", "canUseTool", "useResponsesApi", "isCloudProvider", "useProxy"]
