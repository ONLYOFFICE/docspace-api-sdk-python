...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **tag_name2** | **str**| The tag being checked. Send the same value as the `tagName` query parameter, which is the one the handler reads. | 
...
with docspace_api_sdk.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = docspace_api_sdk.RoomsApi(api_client)
    tag_name2 = 'tag_name_example' # str | The tag being checked. Send the same value as the `tagName` query parameter, which is the one the handler reads.
